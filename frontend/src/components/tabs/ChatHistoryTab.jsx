...
import { Search, Filter, RefreshCw, Clock, MessageSquare, Hash, Bot, ChevronDown, ChevronRight, Copy } from 'lucide-react';
          ...
          {groupedHistory.map((session) => {
            ...
            return (
              <div key={session.sessionId} className="card p-4 space-y-3">
                ...
                {expanded && (
                  <div className="space-y-3 border-t border-dark-100 pt-3">
                    {session.messages.map((item) => (
                      <div key={item.id} className="bg-dark-50 rounded-lg p-3 space-y-2">
                        <div className="flex items-center justify-between text-xs text-dark-500">
                          <div className="flex items-center gap-1">
                            <Clock className="w-3 h-3" /> {formatDateTime(item.created_at)}
                          </div>
                          <span className={`px-2 py-0.5 rounded-full ${item.role === 'assistant' ? 'bg-primary-50 text-primary-700' : 'bg-dark-100 text-dark-600'}`}>
                            {item.role}
                          </span>
                        </div>
                        <div className="text-sm text-dark-800 whitespace-pre-wrap leading-relaxed">{item.content}</div>
                        <div className="text-xs text-dark-400 flex flex-wrap items-center gap-2">
                          {item.role === 'assistant' && (
                            <>
